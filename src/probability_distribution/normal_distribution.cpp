...
NormalDistribution::NormalDistribution(const MatrixXd mean, const MatrixXd variance)
...
Eigen::MatrixXd NormalDistribution::calcDencityFunction(const MatrixXd input)
{
  ...
  varianceSqrt = variance_.cwiseSqrt().matrix();// array().abs().sqrt();
  MatrixXd fact1(1, 1);
  fact1 = 1 / (sqrt(2 * M_PI) * varianceSqrt.array());

  MatrixXd squeredError(1, 1);
  squeredError(0, 0) = std::pow(input(0, 0) - mean_(0, 0), 2);

  MatrixXd fact2(1, 1);
  fact2(0, 0) = - squeredError(0, 0) / (2 * variance_(0, 0));
