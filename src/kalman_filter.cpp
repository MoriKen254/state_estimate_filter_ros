...
using state_estimate_filter_ros::Particle2;
...
Particle2::Particle2() : state_(MatrixXd::Zero(1, 1)), weight_(MatrixXd::Zero(1, 1)), logDencity_(MatrixXd::Zero(1, 1))
{
}

Particle2::Particle2(MatrixXd state, MatrixXd weight)
  : state_(state), weight_(weight)
{
  logDencity_ = MatrixXd::Zero(1, 1);
}

KalmanFilter::KalmanFilter() : StateEstimateFilter()
{
}

KalmanFilter::KalmanFilter(ros::NodeHandle& nh) : StateEstimateFilter(nh)
{
}
...
void KalmanFilter::initParicles(const int num, const MatrixXd state, const MatrixXd weight)
{
  for(int i = 0; i < num; i++)
  {
    particles_.push_back(Particle2(state, weight));
  }
}

void KalmanFilter::predict(const MatrixXd vec_input_curr)
{
  MatrixXd noise(1, 1);
  ...
  vec_predict_curr_ = system_a_ * vec_estimate_prev_ + system_b_ * vec_input_curr + system_c_ + noise;

  system_noise_var_(0, 0) = 3*3;
  prior_var_ = (system_a_*system_a_*post_var_prev_) + (system_b_*system_b_*system_noise_var_);
...
void KalmanFilter::filter(const MatrixXd vec_observation_curr)
{
  ...
  kalman_gain(0, 0) = prior_var_(0, 0) / (prior_var_ + observe_noise_var_)(0, 0);

  MatrixXd innovation(1, 1);
  innovation = vec_observation_curr - vec_predict_curr_;
  vec_estimate_curr_ = vec_predict_curr_ + kalman_gain * innovation;
  post_var_curr_ = (MatrixXd::Identity(1, 1) - kalman_gain) * prior_var_;

  vec_estimate_prev_ = vec_estimate_curr_;
  vec_predict_prev_ = vec_predict_curr_;
  post_var_prev_ = post_var_curr_;
#if 0
